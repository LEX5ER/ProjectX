...
{
    public async Task<PmAuthorizationContext> GetCurrentAsync(RequestContext requestContext, CancellationToken cancellationToken)
    {
    ...
    private static PmAuthorizationContext Map(IamUserResponse user)
    {
        var hasGlobalProjectsRead = user.HasGlobalFullAccess
        ...
        var hasGlobalProjectsWrite = user.HasGlobalFullAccess
            || user.GlobalPermissions.Contains("projects.write", StringComparer.OrdinalIgnoreCase);
        ...
        return new PmAuthorizationContext(
            user.HasGlobalFullAccess,
            hasGlobalProjectsRead,
            hasGlobalProjectsWrite,
