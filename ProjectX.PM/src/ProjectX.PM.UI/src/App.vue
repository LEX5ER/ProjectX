...
const activeProjectSummary = computed(() => auth.getActiveProject()?.name ?? "No PM project selected");
  ...
  <div class="app-shell" :class="{ 'app-shell-auth': !currentUser }">
    <aside v-if="currentUser" class="side-nav">
      <div class="side-nav-brand">
        ...
        <p class="brand-copy">Source of truth for real projects. Admin actions follow the active IAM project.</p>
      </div>
      ...
      <nav class="side-nav-links" aria-label="Primary">
        ...
        <RouterLink class="side-nav-link" :class="{ 'side-nav-link-active': activeRouteName === 'projects' }" to="/projects">
          Projects
        </RouterLink>
      </nav>
